            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div class="rounded-lg border p-4">
                        ...
                        @php
                            $pDraft = $dashboardStatus['penawaran']['draft'];
                            $pSubmitted = $dashboardStatus['penawaran']['submitted'];
                            $pApproved = $dashboardStatus['penawaran']['approved'];
                            $pRejected = $dashboardStatus['penawaran']['rejected'];
                            $pTotal = max($pDraft + $pSubmitted + $pApproved + $pRejected, 1);
                            $p1 = ($pDraft / $pTotal) * 100;
                            $p2 = $p1 + (($pSubmitted / $pTotal) * 100);
                            $p3 = $p2 + (($pApproved / $pTotal) * 100);
                        @endphp
                        <div class="flex items-center gap-4">
                            <div class="w-24 h-24 rounded-full border"
                                 style="background: conic-gradient(#94a3b8 0% {{ $p1 }}%, #3b82f6 {{ $p1 }}% {{ $p2 }}%, #22c55e {{ $p2 }}% {{ $p3 }}%, #ef4444 {{ $p3 }}% 100%);">
                            </div>
                            <div class="space-y-1">
                                <p>Draft: <strong>{{ $pDraft }}</strong></p>
                                <p>Submitted: <strong>{{ $pSubmitted }}</strong></p>
                                <p>Approved: <strong>{{ $pApproved }}</strong></p>
                                <p>Rejected: <strong>{{ $pRejected }}</strong></p>
                            </div>
                        </div>
                    </div>

                    <div class="rounded-lg border p-4">
                        <h4 class="font-semibold mb-2">Status Purchasing Order</h4>
                        @php
                            $poWait = $dashboardStatus['purchasing_order']['menunggu_upload'];
                            $poDone = $dashboardStatus['purchasing_order']['sudah_upload'];
                            $poTotal = max($poWait + $poDone, 1);
                            $po1 = ($poWait / $poTotal) * 100;
                        @endphp
                        <div class="flex items-center gap-4">
                            <div class="w-24 h-24 rounded-full border"
                                 style="background: conic-gradient(#f59e0b 0% {{ $po1 }}%, #22c55e {{ $po1 }}% 100%);">
                            </div>
                            <div class="space-y-1">
                                <p>Menunggu Upload PO: <strong>{{ $poWait }}</strong></p>
                                <p>Sudah Upload PO: <strong>{{ $poDone }}</strong></p>
                            </div>
                        </div>
                    </div>

                    <div class="rounded-lg border p-4">
                        <h4 class="font-semibold mb-2">Status Invoice</h4>
                        @php
                            $iUnpaid = $dashboardStatus['invoice']['belum_dibayar'];
                            $iPaid = $dashboardStatus['invoice']['sudah_dibayar'];
                            $iTotal = max($iUnpaid + $iPaid, 1);
                            $i1 = ($iUnpaid / $iTotal) * 100;
                        @endphp
                        <div class="flex items-center gap-4">
                            <div class="w-24 h-24 rounded-full border"
                                 style="background: conic-gradient(#f59e0b 0% {{ $i1 }}%, #22c55e {{ $i1 }}% 100%);">
                            </div>
                            <div class="space-y-1">
                                <p>Belum Dibayarkan: <strong>{{ $iUnpaid }}</strong></p>
                                <p>Sudah Dibayarkan: <strong>{{ $iPaid }}</strong></p>
                            </div>
                        </div>
                    </div>

                    <div class="rounded-lg border p-4">
                        <h4 class="font-semibold mb-2">Status Faktur Pajak</h4>
                        @php
                            $fWait = $dashboardStatus['faktur_pajak']['menunggu_upload'];
                            $fUnpaid = $dashboardStatus['faktur_pajak']['belum_dibayar'];
                            $fPaid = $dashboardStatus['faktur_pajak']['sudah_dibayar'];
                            $fTotal = max($fWait + $fUnpaid + $fPaid, 1);
                            $f1 = ($fWait / $fTotal) * 100;
                            $f2 = $f1 + (($fUnpaid / $fTotal) * 100);
                        @endphp
                        <div class="flex items-center gap-4">
                            <div class="w-24 h-24 rounded-full border"
                                 style="background: conic-gradient(#94a3b8 0% {{ $f1 }}%, #f59e0b {{ $f1 }}% {{ $f2 }}%, #22c55e {{ $f2 }}% 100%);">
                            </div>
                            <div class="space-y-1">
                                <p>Menunggu Upload: <strong>{{ $fWait }}</strong></p>
                                <p>Belum Dibayarkan: <strong>{{ $fUnpaid }}</strong></p>
                                <p>Sudah Dibayarkan: <strong>{{ $fPaid }}</strong></p>
                            </div>
                        </div>
                    </div>
                </div>
