        ...
        <div style="{{ $previewContentStyle }}">
            ...
            <div class="mt-4 md:hidden space-y-3">
                @foreach ($penawaran->items as $item)
                    <article class="rounded-xl border border-gray-200 bg-white p-4 text-sm shadow-sm">
                        ...
                        <div class="mt-3 space-y-2">
                            <div class="rounded-lg bg-gray-50 px-3 py-2">
                                <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Rincian</p>
                                <p class="mt-1 text-gray-800">{{ $item->rincian ?: '-' }}</p>
                            </div>

                            <div class="grid grid-cols-2 gap-2">
                                <div class="rounded-lg bg-gray-50 px-3 py-2">
                                    <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Qty</p>
                                    <p class="mt-1 text-gray-800">{{ rtrim(rtrim(number_format($item->qty, 2, '.', ''), '0'), '.') }}</p>
                                </div>
                                <div class="rounded-lg bg-gray-50 px-3 py-2">
                                    <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Satuan</p>
                                    <p class="mt-1 text-gray-800">{{ strtoupper($item->satuan) }}</p>
                                </div>
                            </div>
                        </div>
                    </article>
                    ...
                    <tbody>
                        @foreach ($penawaran->items as $item)
                            <tr class="border-b">
                                ...
                                <td class="py-2 pr-3">{{ $item->rincian ?: '-' }}</td>
