  ...
  <form class="space-y-6" @submit.prevent="onSubmit">
      ...
      <div class="grid gap-6 p-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div>
          ...
          <p class="mt-3 max-w-2xl text-sm leading-7 text-slate-600">
            Form ini mengikuti alur lama, tapi tampilannya sudah dibawa ke Inertia + Vue agar konsisten dengan arah baru aplikasi.
          </p>
        </div>
    ...
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      <div v-else class="block rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <p class="mt-1 text-xs text-slate-500">Mitra dibaca dari data backend.</p>
      </div>
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
      <a :href="cancelTo" class="button-ghost justify-center">
        Batal
      </a>
      <button type="submit" class="button-primary" :disabled="busy">
        {{ busy ? 'Menyimpan...' : submitLabelText }}
      </button>
    </div>
  </form>
</template>
...
const form = reactive(createState(props.initialValues ?? null))
...
const selectedCustomer = computed(() => (props.customers ?? []).find((customer) => customer.nama === form.to_company) ?? null)
...
const submitLabelText = computed(() => props.submitLabel || (props.mode === 'create' ? 'Simpan Penawaran' : 'Simpan Perubahan'))
...
const subtotal = computed(() => form.items.reduce((sum, item) => sum + itemAmount(item), 0))
const taxAmount = computed(() => subtotal.value * (toNumber(form.tax_percent) / 100))
