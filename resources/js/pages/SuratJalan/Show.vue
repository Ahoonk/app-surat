    ...
    <div class="space-y-6">
      <section v-if="detail" class="panel overflow-hidden">
        <div class="grid gap-6 p-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            <p class="text-sm uppercase tracking-[0.3em] text-slate-500">Detail Surat Jalan</p>
          ...
          <div class="rounded-[22px] bg-gradient-to-br from-slate-950 to-cyan-900 p-5 text-white">
            <p class="text-xs uppercase tracking-[0.3em] text-cyan-200/70">Customer</p>
            <h3 class="mt-3 break-words text-2xl font-semibold">
              {{ detail.invoice?.penawaran?.to_company || detail.invoice?.penawaran?.customer_nama || '-' }}
            </h3>
            <p class="mt-2 text-sm text-slate-200/80">{{ detail.invoice?.penawaran?.status ?? '-' }}</p>
          </div>
        </div>
      </section>
        ...
        <article class="panel p-6">
          ...
          <div class="mt-4 space-y-3 text-sm">
            <div class="rounded-2xl bg-slate-50 px-4 py-3">
              <p class="text-slate-500">Pemberi</p>
              <p class="font-medium text-slate-900">{{ detail.pemberi_nama || '-' }}</p>
              <p class="text-slate-500">{{ detail.pemberi_jabatan || '-' }}</p>
            </div>
            <div class="rounded-2xl bg-slate-50 px-4 py-3">
              <p class="text-slate-500">Alamat Pemberi</p>
              <p class="font-medium text-slate-900">{{ detail.pemberi_alamat || '-' }}</p>
            </div>
            <div class="rounded-2xl bg-slate-50 px-4 py-3">
              <p class="text-slate-500">Penerima</p>
              <p class="font-medium text-slate-900">{{ detail.penerima_nama || '-' }}</p>
              <p class="text-slate-500">{{ detail.penerima_hp || '-' }}</p>
            </div>
            <div class="rounded-2xl bg-slate-50 px-4 py-3">
        ...
        <article class="panel p-6">
          ...
          <div class="mt-4 space-y-3 text-sm">
            ...
            <div class="rounded-2xl bg-slate-50 px-4 py-3">
              ...
              <p class="font-medium text-slate-900">{{ detail.invoice?.faktur_pajak?.dokumen_name ?? '-' }}</p>
            </div>
            <div class="rounded-2xl bg-slate-50 px-4 py-3">
              <p class="text-slate-500">Berita Acara</p>
              <p class="font-medium text-slate-900">{{ detail.invoice?.berita_acara?.nomor ?? '-' }}</p>
            </div>
          </div>
        </article>
      </section>
      ...
      <section class="flex flex-col gap-3 sm:flex-row sm:justify-between">
        <a href="/surat-jalan" class="button-ghost justify-center sm:w-auto">Kembali ke daftar</a>
...
const response = await api.get(`/api/surat-jalans/${props.suratJalanId}`)
detail.value = response.data?.data ?? null

const pdfLink = computed(() => `/surat-jalan/${props.suratJalanId}/pdf?download=1`)
...
async function sendEmail() {
  ...
  try {
    const response = await api.post(`/api/surat-jalans/${detail.value.id}/send`)
    actionMessage.value = response.data?.message ?? 'Surat jalan berhasil dikirim.'
  } catch (error) {
    actionError.value = extractApiMessage(error, 'Gagal mengirim surat jalan.')
  } finally {
    busy.value = false
  }
}
