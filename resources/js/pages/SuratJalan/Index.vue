              ...
              <tr v-for="item in rows" :key="item.id" class="border-t border-slate-200">
                ...
                <td class="px-5 py-4">
                  <div class="flex flex-wrap gap-2">
                    <a :href="`/surat-jalan/${item.id}`" class="button-ghost px-3 py-2 text-sm">Detail</a>
        ...
        <div class="grid gap-4 p-4 lg:hidden">
          ...
          >
            ...
            <div class="mt-4 grid grid-cols-2 gap-2">
              <a :href="`/surat-jalan/${item.id}`" class="button-ghost w-full px-3 py-2 text-sm">Detail</a>
              <a :href="pdfLink(item.id)" target="_blank" class="button-primary w-full px-3 py-2 text-sm">PDF</a>
              <button
                type="button"
...
async function loadRows() {
  const response = await api.get('/api/surat-jalans')
...
function pdfLink(id) {
  return `/surat-jalan/${id}/pdf?download=1`
}
...
async function sendEmail(item) {
  ...
  try {
    const response = await api.post(`/api/surat-jalans/${item.id}/send`)
