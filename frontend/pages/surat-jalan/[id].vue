<template>
  <div class="space-y-6">
    ...
    <section class="flex flex-col gap-3 sm:flex-row sm:justify-between">
      <NuxtLink to="/surat-jalan" class="button-ghost justify-center sm:w-auto">
        Kembali ke daftar
      </NuxtLink>
...
import type { SuratJalanDetail } from '~/types/api'
...
const { data } = await useAsyncData(`surat-jalan-${id.value}`, () => apiFetch<{ data: SuratJalanDetail }>(`/api/surat-jalans/${id.value}`))
const detail = computed(() => data.value?.data ?? null)
const busy = ref(false)

const pdfLink = computed(() => `${backendBase}/surat-jalan/${id.value}/pdf?download=1`)

async function sendEmail() {
  busy.value = true

  try {
    await apiFetch(`/api/surat-jalans/${id.value}/send`, {
      method: 'POST',
    })
  } catch (error: any) {
    alert(error?.data?.message ?? 'Gagal mengirim surat jalan.')
  } finally {
    busy.value = false
  }
}
