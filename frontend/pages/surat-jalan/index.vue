    ...
    <section class="panel p-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 class="section-title mt-2">Surat Jalan</h2>
          <p class="mt-3 text-sm text-slate-600">Dokumen pengiriman yang tersinkron dari invoice, sama seperti alur backend lama.</p>
        </div>
        <div class="text-sm text-slate-500">
          {{ rows.length }} surat jalan
        </div>
      </div>
    </section>
            ...
            <tr v-for="item in rows" :key="item.id" class="border-t border-slate-200">
              ...
              <td class="px-5 py-4">
                <div class="flex flex-wrap gap-2">
                  <NuxtLink :to="`/surat-jalan/${item.id}`" class="button-ghost px-3 py-2 text-sm">
                    Detail
                  </NuxtLink>
      ...
      <div class="grid gap-4 p-4 lg:hidden">
        ...
        >
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <p class="text-[11px] uppercase tracking-[0.28em] text-slate-500">Surat Jalan</p>
          ...
          <div class="mt-4 grid grid-cols-2 gap-2">
            <NuxtLink :to="`/surat-jalan/${item.id}`" class="button-ghost w-full px-3 py-2 text-sm">
              Detail
            </NuxtLink>
...
import type { ApiListResponse, SuratJalanSummary } from '~/types/api'
...
const { data } = await useAsyncData('surat-jalans', () => apiFetch<ApiListResponse<SuratJalanSummary>>('/api/surat-jalans'))
...
function pdfLink(id: number): string {
  return `${backendBase}/surat-jalan/${id}/pdf?download=1`
}

async function sendEmail(item: SuratJalanSummary) {
  ...
  try {
    const response = await apiFetch<{ message?: string }>(`/api/surat-jalans/${item.id}/send`, {
      method: 'POST',
    })

    actionMessage.value = response.message ?? 'Surat jalan berhasil dikirim.'
  } catch (error: any) {
